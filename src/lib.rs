...
// Пока что состояние Вселенной представляется в виде вектора ячеек. Чтобы сделать это удобочитаемым человеком, давайте реализуем базовый рендеринг текста. Идея состоит в том, чтобы записать вселенную построчно в виде текста и для каждой живой ячейки вывести символ Юникода ◼(«черный средний квадрат»). Для мертвых клеток мы будем печатать ◻ («белый средний квадрат»).
// Реализуя этот Display признак из стандартной библиотеки Rust, мы можем добавить способ форматирования структуры в удобном для пользователя виде. Это также автоматически даст нам to_string метод.
